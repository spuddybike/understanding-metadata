...
??? success "Answers"
    ...
    **True or false...**
    ...
    - An individual team or organisation can create a standard **FALSE** - Creating a metadata schema does not make it a metadata standard, a schema becomes a standard only when it is formally recognised and adopted by a wider community.
    - A metadata standard never changes once it has been created. **FALSE** - A metadata standard can be based on a schema — TRUE
A metadata standard provides the guiding framework, while schemas are often implementations of that standard, defining how its elements are structured and applied in practice.
    - All metadata standards use the same set of elements. **FALSE** - Metadata standards differ by discipline and purpose.
