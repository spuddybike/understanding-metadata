...
!!! abstract ""
    ...
    You're conducting a study exploring the behaviours of young people in 1965-1970 in the Northern regions of England. To do this, you need to use secondary data (data that already exists) from that time period. 
...
As metadata can describe different areas of your research, we could group the above questions by their focus.
...
!!! abstract ""
    ...
    Using the study metadata available on [on CLOSER Discovery,](https://discovery.closer.ac.uk/Item/uk.cls.ncds/524c8f92-493f-4660-90c4-0aa7ac3d2640/) try answering some of the questions above.
...
While the study metadata gives us a useful overview to understand a research project, having information about the individual dataset can help us further assess whether it's data is relevant to our work. Dataset metadata is particularly useful when a study produces more than one dataset as the datasets may be produced at different times, by different researchers or have a different research focus.
...
!!! abstract ""
    ...
    You can explore metadata for the unlabelled dataset [on CLOSER Discovery here](https://discovery.closer.ac.uk/item/uk.cls.ncds/a52ee654-efa8-4839-a545-fd82bec63e71)<sup>[9]</sup>. Using this metadat, try answering some of the questions above.

    Through exploring the dataset metadata, we know that the dataset was produced in 1969 and it was collected via a survey. This falls within our brief. However, what further information could help us assess whether we wanted to use the data?

### Variable metadata

Accessing datasets can be time consuming and expensive. You may examine the study and dataset metadata and believe it is relevant to your project. However, when you then access the dataset, it doesn't contain the data you were expecting for example, it has a different focus within the topic area or uses measurements that are not relevant for your work.

Having information about the data inside a dataset before you access it allows you to make an informed decision about its relevance to your work. This makes the process more efficient and cost effective.

Variable metadata is a key tool that enables us to do this. We use the term variable all the time, but what is it's definition?
...
??? question "What are the variables in the unlabelled dataset?"
    ...
    Simply put, variables are the header of the columns in tabular data (data stored in rows and columns).

What information could we collect about the vairables to helps us understand whether the data is relevant for our work?
