...
>[!NOTE]
> HM This is taken from the Introduction to Metadata course. Need to decide if this works better here or not. 
...
As we explored in [unit 2.5](<2.5 Metadata standards.md>), using a metadata standard or schema can help ensure our metadata is consistent and interoperable. It also saves us time as it provides defined elements and a clear structure so we don't have to create it from scratch.

>[!NOTE]
>BO - need to change the "unit 2.5" link
...
For the following exercises, we will use a simple online generator developed by Nick Steffel. The aim of the tool was to simplify Dublin Core metadata creation by providing an online form to input information which can then be downloaded in a machine readable format.
...
??? warning "Challenges of Dublin Core Simple Metadata Generator"
    ...
    While there are some guidelines around what information you should include for each metadata field, the simple generator does not refer to controlled vocabularies. This means metadata can be presented in different ways. This could decrease the quality of your metadata as well as it's interoperability. 
...
!!! question "How could we improve the quality of this metadata? What tools could we use?"
...
:arrow_right: If you're currently working on a research project or have a resource you would like to create metadata for, try using the Dublin Core Metadata Simple or Advanced Generator to create study level metadata.

>[NOTE]
>SW - In 4.2 we mentioned that we couldn’t recommend any one specific software to create metadata. I think we should do that here too  – especially as we delve into the challenges of Dublin Core v Dublin Core Advanced Metadata Generator – at the end of this section we could summarise to say that this is an example of the challenges to consider when deciding which metadata generator to use.
>BO - agree with SW, but maybe we just need to strip back some of the detail comparing the simple vs. the advanced metadata generator, and instead just say that we're using this tool as an example because it's cross-discipline and widely used, and that you can either used the simple version which allows you to do XYZ, or the advanced version which has XYZ additional emlements/features. Then as Sarah said, say that these are some of the things/features etc to look out for when deciding what tool to use.
